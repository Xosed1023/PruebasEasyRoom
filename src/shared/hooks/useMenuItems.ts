...
export const useMenuItems = (rolName, withEasyrewards, navigate, openModal) => {
    return useMemo<{ label: string; icon: IconNameType; onClick: () => void }[]>(() => {
        if (rolName === RoleNames.valet) {
            return [...(withEasyrewards ? [
                { label: "Easyrewards", icon: "giftFill", onClick: openModal },
                { label: "Control vehicular", icon: "car", onClick: () => navigate("/u/control-vehicular") }
            ] : [])]
        }
        if (rolName === RoleNames.restaurante){
            return [...(withEasyrewards ? [
                { label: "Easyrewards", icon: "giftFill", onClick: openModal },
            ] : [])]
        }
