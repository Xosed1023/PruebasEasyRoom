...
import { getLabelTime } from "../../historial/helpers/time"
...
export const useGenTable = () => {
    ...
    const genTable = ({
        ...
        return genTableTodas(UTCStringToLocalDate, ordenes, handlePrint)
    }
            ...
            value: [
                ...
                { value: orden?.turno?.nombre || "-" },
                {
                    value: orden.fecha_registro
                        ? `${formatDateComplitSlash(getExactDate(orden?.fecha_registro), false)}  
                        ${getLabelTime(getExactDate(orden?.fecha_registro))}`
                        : "-",
                },
                {
                    value: (
                ...
                },
                {
                    ...
                    value: detallesPago.length
                        ? getPaymentMethod(detallesPago as DetallePago[])
                        : "-",
                },
                {
                    value: (
                        ...
                        <div
                            onClick={(e) => {
                                ...
                                if (detallesPago.length > 0) {
                                    const lastTicket =
                                        orden.pagos?.[orden.pagos.length - 1]?.ticket_id || ""
