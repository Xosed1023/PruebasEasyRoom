...
const SupervisionPending = ({
    ...
    return (
        ...
        >
            ...
            <RoomCardBody>
                {elapsedMinutes >= 10 ? (
                    <span className="room-card--xs--supervision-pending__timer--timeout">{elapsedTime}</span>
                ) : (
                    <span className="room-card--xs--supervision-pending__timer">{elapsedTime}</span>
                )}
                <span className="room-card--xs--supervision-pending__description">{roomTypeName}</span>
            </RoomCardBody>
        </Wrapper>
    )
}
