...
const RoomCards = () => {
    ...
    return (
        <div
            className="room-cards__wrapper__wrapper scrollbar__light-fat"
            style={{
                gridTemplate: `repeat(${roomsDimensions?.x}, 1fr) / repeat(${roomsDimensions?.y}, 1fr)`,
            }}
        >
            <div className="room-cards__wrapper">
                {rooms?.map((room) => (
                    <RoomCard
                        style={{
                        ...
                        roomServiceTimeEnd={room.estado === RoomStatus.occupied ? new Date().toISOString() : undefined}
                        roomServiceTimeStart={room.estado === RoomStatus.occupied ? new Date().toISOString() : undefined}
                        ...
                        comentarioUltimoBloqueo={
                            room?.comentario_estado || (RoomStatus.blocked && "Habitación cancelada") || "No comentarios"
                        }
                        cleaningTimeEnd={
                        ...
                        supervisionColaboradorPhotoUrl={room?.colaborador_tareas_sin_finalizar?.[0]?.colaborador?.foto}
                        ...
                        occupiedExtraHours={
                            ...
                            times(room.ultima_renta?.hospedajes_extra || 0, room.ultima_renta?.tarifa?.duracion_renta || 0)
                        }
                        ...
                        reservedDetails={
                            room.ultima_reserva?.reserva?.comentarios[room.ultima_reserva?.reserva?.comentarios?.length - 1]
                                ?.comentario
                        }
