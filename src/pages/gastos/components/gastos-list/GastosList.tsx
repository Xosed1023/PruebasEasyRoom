...
const GastosList = ({ numFolio = 0, search = "", date = "", year = "", reload, refetchGastos }: Props) => {
            ...
            .then(({ data }) => {
                if (data?.gastos) {
                    ...
                    const array: GastosRow[] =
                        profile === RoleNames.recepcionista
                            ? getRowsRecepcion(dataFilter, usuario_id)
                            : getRows(dataFilter)
                ...
                value: [
                    {
                        value: (
                            <p className={gastoData.eliminado ? "gastos-table-row-eliminado" : ""}>
                                {gastoData?.usuario?.roles[0]?.nombre === RoleNames.admin ||
                                gastoData?.usuario?.roles[0]?.nombre === RoleNames.superadmin
                                    ? "A-"
                                    : "R-"}
                                {gastoData?.folio}
                            </p>
                    ...
                    },
                    {
                        value: (
                            <p className={gastoData.eliminado ? "gastos-table-row-eliminado" : ""}>
                                {formatCustomDate(fechaRegistro, "DD/MMM/YY")}
                                <br />
                                {formatCustomDate(fechaRegistro, "hh:mm a")}
                            </p>
                        ),
                    },
                    {
                        value: (
                            <p className={gastoData.eliminado ? "gastos-table-row-eliminado" : ""}>
                                {formatCustomDate(gastoData.fecha_gasto, "DD/MMM/YY")}
                            </p>
                        ),
                    },
                    {
                ...
                value: [
                    ...
                    },
                    {
                        value: (
                            <p className={gastoData.eliminado ? "gastos-table-row-eliminado" : ""}>
                                {fechaRegistro
                                    ? `${formatCustomDate((fechaRegistro), "DD/MMM/YY")} ${formatCustomDate((fechaRegistro), "hh:mm a")}`
                                    : ""}
                            </p>
                        ),
                    },
    ...
    const headersAdmin = useMemo(() => {
        ...
        return [
            ...
            ...(profile === RoleNames.admin || profile === RoleNames.superadmin
                ? tableItems.headers
                : tableItemsRecepcion.headers),
        ]
