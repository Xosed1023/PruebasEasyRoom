...
const VentaHabitacion = () => {
    ...
    const getCurrentDate = (): Date => currentDate || new Date()
    ...
    const onSubmit = async (data) => {
        ...
        if (data.type === "hotel") {
            ...
            const endDate = setHHMMSS({
                startDate: data.date[0],
                newHour: tarifaSeleccionada?.hora_checkout || "",
                isNewHourInUTC: true,
            })
            const endNextDayTarifa = endDate < getCurrentDate()

            //formato HH:mm:ss para fechaRegistro y horaCheckOut
            const fechaRegistro = new Date(data.date[0]).toTimeString().split(" ")[0]
            const horaCheckOut = tarifaSeleccionada?.hora_checkout?.split("-")[0] || "" //hora_checkout no viene convertido a utc, viene con la hora del hotel

            //formato de hora en número entero de fechaRegistro y horaCheckOut
            const horaIntRegistro = Number(fechaRegistro?.split(":")?.[0] || 0)
            const horaIntCheckOut = Number(horaCheckOut?.split(":")?.[0] || 0)

            formattedDate =
                horaIntRegistro < horaIntCheckOut && data?.date?.[0]?.toISOString() === data?.date?.[1]?.toISOString()
                    ? endDate.toISOString()
        ...
        const fecha_checkin = localDateToUTCString(
            setHHMMSS({
                ...
                newHour: tarifaSeleccionada?.hora_checkin || "",
                isNewHourInUTC: true,
            })
        )

        // Se agregó el campo “fecha_checkin” cuando el tipo es hotel, en horario que se registre la venta
        const datos_renta: CreateRentaInput = {
            comentarios: [],
            fecha_checkin,
            ...(data.type === "hotel"
                ? { fecha_fin: formattedDate, fecha_checkin: getCurrentDate().toISOString() }
                : {}),
            habitacion_id,
            early_checkin: data.earlyCheckIn,
            ...(propinasFormatted.length
