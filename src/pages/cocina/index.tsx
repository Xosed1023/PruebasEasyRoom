...
import IncompleteOrderModal from "./components/IncompleteOrderModal/IncompleteOrderModal"
...
function Cocina(): JSX.Element {
    ...
    const [usuario_autorizo_id, setUsuario_autorizo_id] = useState("")
    ...
    const { onConfirmChangeOrderState, onConfirmCancelacion, liberarDetallesOrden, confirmFinalizarOrder, showIncompleteOrderModal, setshowIncompleteOrderModal } = useChangeOrderState({
        onClose: () => unselectAll(),
        closeAuthCocinaModal: () => closeAuthCocinaModal()
    })
    const { unselectAll, closeAuthCocinaModal } = useToggleSelectOrder()
        ...
        authModal: (
            <AuthModalSelector
                ...
                onAuthFilled={(usuario_autorizo_id) => {
                    setUsuario_autorizo_id(usuario_autorizo_id || "")
    ...
    return (
        ...
        >
            ...
            {Modal}
            {showIncompleteOrderModal && <IncompleteOrderModal onConfirm={() => confirmFinalizarOrder(usuario_autorizo_id, () => {
                setUsuario_autorizo_id("");
                unselectAll();
            } )} onCancel={() => {
                setUsuario_autorizo_id("");
