...
function CaratulaFecha(): JSX.Element {
    ...
    const handlePdf = () => {
        ...
        if (fecha_corte_inicio && fecha_corte_fin) {
            ...
            const fechas = range
                ? `${formatDateComplitSlash(fecha_inicial, false)} - ${formatDateComplitSlash(fecha_final, false)}`
                : formatDateComplitSlash(fecha_inicial, false)
    ...
    return (
        <Screen
            ...
            title={range ? "Corte por fechas" : `Día ${formatDateComplitSlash(fecha_inicial, false)}`}
            close={true}
            headerRight={
                data.length > 0 ? (
        ...
        >
            {range && (
                <p className="caratulas-f__subtitle">
                    Fechas:{" "}
                    <span>
                        {`${formatDateComplitSlash(fecha_inicial, false)} al ${formatDateComplitSlash(
                            fecha_final,
                            false
                        )}`}
                    </span>
