...
const GastosAdministrativos = ({ caratula }: { caratula: CaratulaPeriodoType }) => {
    const empty = "$0.00"
    return (
        <Table style={{ marginTop: "10px", border: "1px solid #e3e3e3" }}>
            ...
            {caratula?.resumen_gastos_administrativos.detalles?.map((categoria, index) => (
            ...
            >
                ...
                <Cell width="15%" textStyle={{ fontSize: 9 }}>
                    {caratula?.resumen_gastos_administrativos.totales.total_gasto
                        ? formatCurrencyToFixed(caratula?.resumen_gastos_administrativos.totales.total_gasto)
                        : empty}
                </Cell>
                <Cell width="15%" textStyle={{ fontSize: 9 }}>
                    {caratula?.resumen_gastos_administrativos.totales.porcentaje_venta
                        ? caratula?.resumen_gastos_administrativos.totales.porcentaje_venta
                        : "0"}
                </Cell>
                <Cell width="17%" textStyle={{ fontSize: 9 }}>
                    {caratula?.resumen_gastos_administrativos.totales.promedio_habitacion
                        ? formatCurrencyToFixed(caratula?.resumen_gastos_administrativos.totales.promedio_habitacion)
                        : empty}
                </Cell>
            </Row>
        </Table>
