...
const GastosCategoriasCaratula = ({ caratula }: { caratula: CaratulaPeriodoType }) => {
    const empty = "$0.00"
    return (
        <Table style={{ marginTop: "10px", border: "1px solid #e3e3e3" }}>
            ...
            {caratula?.resumen_gastos_hotel.detalles?.map((categoria, index) => (
            ...
            <Row
                ...
                backgroundColor={caratula?.resumen_gastos_hotel.detalles.length % 2 !== 0 ? "#f4f6f8" : ""}
            >
                <Cell width="53%" textStyle={{ fontWeight: 500, fontSize: 9 }}>
                    Total
                </Cell>
                <Cell width="15%" textStyle={{ fontSize: 9 }}>
                    {caratula?.resumen_gastos_hotel.totales.total_gasto
                        ? formatCurrencyToFixed(caratula?.resumen_gastos_hotel.totales.total_gasto)
                        : empty}
                </Cell>
                <Cell width="15%" textStyle={{ fontSize: 9 }}>
                    {caratula?.resumen_gastos_hotel.totales.porcentaje_venta
                        ? caratula?.resumen_gastos_hotel.totales.porcentaje_venta + ""
                        : "0"}%
                </Cell>
                <Cell width="17%" textStyle={{ fontSize: 9 }}>
                    {caratula?.resumen_gastos_hotel.totales.promedio_habitacion
                        ? formatCurrencyToFixed(caratula?.resumen_gastos_hotel.totales.promedio_habitacion)
                        : empty}
                </Cell>
            </Row>
        </Table>
...
export default GastosCategoriasCaratula
