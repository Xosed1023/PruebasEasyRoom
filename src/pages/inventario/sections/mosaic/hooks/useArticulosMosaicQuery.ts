...
import { FilterSortState, TODOS_ALMACEN_FILTER } from "src/pages/inventario"
import { useProfile } from "src/shared/hooks/useProfile"

const useArticulosMosaicQuery = ({
    page,
    almacenSelected,
    almacenes,
    filterSortState,
    ...
    const articulosRef = useRef<{
        [key: string]: { articulos: AlmacenArticulo[]; totalPages: number; currentPage: number }
    }>({})
    ...
    useEffect(() => {
        ...
        getArticulos({
            variables: {
                hotel_id,
                almacen_id: filterSortState?.almacenFilters?.filter((f) => f.filter.length).length
                    ? filterSortState?.almacenFilters.map((f) => f.filter)
                    : almacenSelected === TODOS_ALMACEN_FILTER.label
                    ? almacenes
                    : [almacenSelected],
                pagination_options: {
                    take: 12,
            ...
            const updatedArticulos = {
                ...
                [almacenSelected]: {
                    ...
                    articulos: [
                        ...articulos?.[almacenSelected]?.articulos || [],
...
export default useArticulosMosaicQuery
