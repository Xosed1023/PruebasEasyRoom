...
const Reports = () => {
    ...
    const { getSelectedSection } = reportSelectedHandler()

    const initialSection = getSelectedSection() || ReportsOptions.habs

    const [sectionSelected, setsectionSelected] = useState<{ title: string; value: string }>(initialSection)
    ...
    return (
            ...
            headerLeft={
                <HeaderLeft>
                    {(dateFilter?.length || 0) > 1 && areSameDay(dateFilter?.[0], dateFilter?.[1])
                        ...
                        : dateFilter?.length === 2
                        ? `${formatCustomDate((dateFilter![0]), "DD/MMM/YY")} al 
                    ${formatCustomDate((dateFilter![1]), "DD/MMM/YY")}`
                        : ""}
                </HeaderLeft>
            }
            headerRight={
                <>
                    {sectionSelected.value === ReportsSubSections.historialEnergeticos && <AgregarEnergeticoButton />}
        ...
        >
            {sectionSelected.value === ReportsSubSections.historialLimpieza ? (
                ...
                <Matriculas apiDateFilter={apiDateFilter} canDownload canDelete />
            ) : null}
        </Screen>
    )
}
