...
const HistorialMantenimientos = ({ apiDateFilter }: { apiDateFilter: string[] | null }) => {
    ...
    const tableItems: FlexibleTableRow[] = data?.respuesta.tabla_manttos?.map((hab) => ({
        value: [
            ...
            { value: hab.comentarios, className: "reports-cell" },
        ],
    }))
    ...
    useEffect(() => {
        setSelectedSection(ReportsOptions.mantenimiento)
    }, [])
    ...
    return (
        <ReportWrapper height="calc(100dvh - 125px)">
            {load ? (
                ...
                <CardReports>
                    <CardReport title="Total de mantenimientos" value={data.dashboard.total_manttos || "0"} />
                    <CardReport
