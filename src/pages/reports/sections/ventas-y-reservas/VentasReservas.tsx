...
import { formatCurrency, formatCurrencyToFixed } from "src/shared/hooks/formatCurrency"
...
import useMovimientoHeader from "./hooks/useMovimientoHeader"
...
const VentasReservas = ({ apiDateFilter }: { apiDateFilter: string[] | null }) => {
    ...
    const movimientoHeader = useMovimientoHeader<Stringify<TablaVentasReserva>>({
        headerValue: "movimiento",
        valueToDisplay: "Movimiento",
    })
    ...
    const formattedMetaValues = useMemo<{
        ...
        () => ({
            filterValues,
            sortValue,
            headerValues: ["numero_habitacion", "tipo_habitacion", "fecha_registro", "origen", "movimiento"],
        }),
    ...
    const tableItems: FlexibleTableRow[] = data?.respuesta.tabla_ventas_reservas?.map((hab) => ({
        value: [
            { value: capitalize(hab.origen), className: "reports-cell" },
            { value: formatText(hab.movimiento), className: "reports-cell" },
            ...
            { value: formatCurrencyToFixed(hab.precio), className: "reports-cell" },
    ...
    useEffect(() => {
        setSelectedSection(ReportsOptions.ventasHabYReservas)
    }, [])
