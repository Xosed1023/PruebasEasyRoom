...
const ArticulosVendidos = ({ apiDateFilter }: { apiDateFilter: string[] | null }) => {
    ...
    const articuloHeader = useArticuloHeader<Stringify<TablaVentasArticulo>>({
        ...
        valueToDisplay: ARTICULO_HEADER_VALUE,
    })
    ...
    const tableItems: FlexibleTableRow[] = data?.respuesta.tabla_ventas_articulos?.map((v, index) => ({
        value: [
            ...
            { value: v.Precio, className: "reports-cell" },
            { value: v.Cantidad, className: "reports-cell" },
            { value: formatCurrency(Number(v.Total)), className: "reports-cell" },
            ...
            {load ? (
                ...
                <CardReports>
                    <CardReport
                        ...
                        value={formatCurrency(Number(data?.dashboard?.total_ventas   || 0))}
                    />
                    <CardReport title={"Total de artículos"} value={formatCurrency(Number(data?.dashboard?.cantidad_articulos))} />
                    <CardReport title={"SKUs"} value={formatCurrency(Number(data?.dashboard?.contador_sku))} />
                    <CardReport
                        title={"Ticket promedio"}
                        value={formatCurrency(Number(data?.dashboard?.ticket_promedio))}
                    />
                </CardReports>
            )}
