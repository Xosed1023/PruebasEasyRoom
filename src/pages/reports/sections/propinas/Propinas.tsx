...
const Propinas = ({ apiDateFilter }: { apiDateFilter: string[] | null }) => {
    ...
    const tableItems: FlexibleTableRow[] = data?.respuesta.tabla_propinas_pagadas?.map((hab) => ({
        value: [
            ...
            { value: formatCurrencyToFixed(Number(hab.utilidad_por_puntos) || 0), className: "reports-cell" },
            { value: formatCurrencyToFixed(Number(hab.comision_bancaria) || 0), className: "reports-cell" },
            { value: formatCurrencyToFixed(Number(hab.pago_a_colaborador)), className: "reports-cell" },
        ],
    }))
    ...
    useEffect(() => {
        setSelectedSection(ReportsOptions.propinas)
    }, [])
            ...
            {load ? (
                ...
                <CardReports>
                    <CardReport
                        ...
                        value={formatCurrencyToFixed(Number(data?.dashboard?.total_propinas_pagadas || 0))}
                    />
                    <CardReport title={"No. de pagos"} value={data?.dashboard?.contador_propinas_pagadas || 0} />
                    <CardReport
                        title={"Promedio de propinas"}
                        value={formatCurrencyToFixed(Number(data?.dashboard?.promedio_propinas_pagadas) || 0)}
                    />
                    <CardReportDouble
                        title1={"Utilidad por puntos"}
                        value1={formatCurrencyToFixed(Number(data?.dashboard?.total_utilidad_por_puntos) || 0)}
                        title2={"Puntos a pagar"}
                        value2={formatCurrencyToFixed(Number(data?.dashboard?.total_puntios_a_pagar) || 0)}
                    />
                    <CardReport title={"Comisión bancaria"} value={formatCurrencyToFixed(Number(data?.dashboard?.total_comisiones_bancarias) || 0)} />
                </CardReports>
            )}
